...
public class DataControl {
	 ...
	 */
	public static Long stringToLongIDCompany(Connection conn, String computerManufacturer, Scanner sc ){
		boolean isOK= false;
		...
		if(!computerManufacturer.equals("")){
			...
			while (!isOK) {
				try(Statement stmt = DatabaseExchange.databaseStatement(conn)) {
					while (!DatabaseExchange.isIdOkCompany(stmt, computerManufacturer)) {
	 ...
	 */
	public static Long stringToLongIDComputer(Connection conn,String computerID, Scanner sc ){
		boolean isOK= false;

		//tant que l'ID n'existe pas ou n'est pas valide
		while (!isOK) {
			try (Statement stmt = DatabaseExchange.databaseStatement(conn)){
				while (!DatabaseExchange.isIdOkComputer(stmt, computerID) ) {
